...
rst                              
  ...
  wire [9:0] counter_out_w;
  counter_1024 switch_ctrl(.in_start(in_start_stage1), .counter_out(counter_out_w), .clk(clk), .rst(rst));
...
module  mem_addr_gen_dp4_mem0_per4(
counter_in,                              
clk,                             
rst,                             
addr_out                            
  ...
  assign word_counter_w[0] = word_counter[0];    
      ...
      assign addr_out[7:4] = word_counter_w[1][0:3];    
      assign ms_word_high_w = ms_word_high;    
  assign addr_out[9:8] = ms_word_high_w[1:0];    
      assign ms_word_low_w = ms_word_low;    
  assign addr_out[9:8] = ms_word_low_w[0:1];   
  ...
  begin                            
    if(rst) begin                    
      ...
      ms_word_high <= (counter_in[6:0] == {7{1'b1}}) + ms_word_high; 
      end
  end                              
...
rst                              
  ...
  mem_addr_gen_dp4_mem0_per4 top_mem_addr_gen_inst (.clk(clk),.rst(rst),.addr_out(mem_addr_out_w), .counter_in(counter)); 
...
rst                              
  ...
  mem_addr_ctrl_dp4_per4 addr_gen_inst(.in_start(in_start), .wen_out(wen_wire), .out_start(out_start_wire), .mem_addr_out_0(addr_wire_0), .mem_addr_out_1(addr_wire_1), .mem_addr_out_2(addr_wire_2), .mem_addr_out_3(addr_wire_3), .clk(clk), .rst(rst));

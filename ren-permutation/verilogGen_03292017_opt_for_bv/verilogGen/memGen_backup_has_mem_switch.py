...
def genAddrGen(fileName, dp, sizeN, addrVec):
    ...
    with open(fileName, 'a') as wrFile:
        ...
        wrFile.write("assign out_start = (rom_addr_0["+str(romAddrWidth[0]-1)+":0] == "+str(len(addrVec[0])-1)+"); \n"  ) 
        wrFile.write("\n  ")
        #Update ctrl_out cycle by cycle
        wrFile.write("always@(posedge clk)             \n  ")
        wrFile.write("begin                            \n    ")
        ...
        wrFile.write("if (rom_addr_0["+str(romAddrWidth[0]-1)+":0] == "+str(len(addrVec[0])-1)+") begin  \n        ")
        wrFile.write("addr_updating <= 1'b0;                 \n        ")
        for i in range(0, numWireGroup):
            wrFile.write("rom_addr_"+str(i)+" <= "+str(sameSizeMap.keys()[i])+"'b0;    \n        ")
        wrFile.write("end\n      ")
...
def genMemStage(fileName, dp, sizeN, dataWidth, addrVec, regOut):
    ...
    memSize = []; addrWidth = []
    for i in range(dp):
        memSize.append(len(addrVec[i])%maxMemSize)
    ...
    with open(fileName, 'a') as wrFile:
        for i in range(0, dp):
            ...
            if(portStyle[i] == "sp"):
                if len(addrVec[i]) == maxMemSize/2 and i>=dp/2:
                    wrFile.write("wire [DATA_WIDTH-1:0] "+"in_switch_"+str(i)+"_out0;              \n  ")
                    wrFile.write("wire [DATA_WIDTH-1:0] "+"out_switch_"+str(i)+"_in0;             \n  \n  ")
                    wrFile.write("wire [DATA_WIDTH-1:0] "+"in_out_switch_"+str(i)+";             \n  \n  ")
                ...
                if len(addrVec[i]) == maxMemSize/2 and i>=dp/2:
                    wrFile.write(".din(in_switch_"+str(i)+"_out0),")
                else:
                    wrFile.write(".din("+wireInName+"["+str(i)+"]), ")

                if len(addrVec[i]) == maxMemSize/2 and i>=dp/2:
                    wrFile.write(".dout(out_switch_"+str(i)+"_in0),")
                else:
                ...
                if len(addrVec[i]) == maxMemSize/2 and i>=dp/2:
                    #Generate input 1-to-2 switch
                    inSwitchName = "switch_1_2"
                    if i==dp/2:
                        wrFile.write("reg ctrl_in_switch; \n        ")
                    wrFile.write(inSwitchName+" #(")
                    wrFile.write(".DATA_WIDTH("+str(dataWidth)+")) \n        ")
                    wrFile.write(" in_switch_inst_"+str(i)+"(")
                    wrFile.write(".ctrl(ctrl_in_switch), ")
                    wrFile.write(".inData_0("+wireInName+"["+str(i)+"]), ")
                    wrFile.write(".outData_0(in_switch_"+str(i)+"_out0),")
                    wrFile.write(".outData_1(in_out_switch_"+str(i)+");\n\n  ")

                    outSwitchName = "switch_2_1"
                    if i==dp/2:
                        wrFile.write("reg ctrl_out_switch; \n        ")
                    wrFile.write(outSwitchName+" #(")
                    wrFile.write(".DATA_WIDTH("+str(dataWidth)+")) \n        ")
                    wrFile.write(" out_switch_inst_"+str(i)+"(")
                    wrFile.write(".ctrl(ctrl_out_switch), ")
                    wrFile.write(".inData_0(out_switch_"+str(i)+"_in0),")
                    wrFile.write(".inData_1(in_out_switch_"+str(i)+"),")
                    wrFile.write(".outData_0("+wireOutName+"["+str(i)+"]);\n\n  ")

                    if i==dp/2:
                        wrFile.write("always@(posedge clk)             \n  ")
                        wrFile.write("begin                            \n    ")
                        wrFile.write("if(rst) begin                    \n      ")
                        wrFile.write("ctrl_in_switch <= 1'b0;             \n      ")
                        wrFile.write("ctrl_out_switch <= 1'b0;            \n      ")
                        wrFile.write("end\n    ")
                        wrFile.write("else begin                        \n      ")
                        wrFile.write("if("+inStartPortName+") begin                    \n      ")
                        wrFile.write("ctrl_in_switch <= 1'b0;             \n      ")
                        wrFile.write("ctrl_out_switch <= 1'b0;            \n      ")
                        wrFile.write("end\n    ")
                        wrFile.write("else if (addr_wire_"+str(i)+" == "+ str(memSize[i]-1) +") begin \n      ")
                        wrFile.write("ctrl_in_switch <= 1'b1;             \n      ")
                        wrFile.write("ctrl_out_switch <= 1'b1;            \n      ")
                        wrFile.write("end\n    ")
                        wrFile.write("end\n  ")
                        wrFile.write("end                              \n")

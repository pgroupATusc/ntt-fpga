...
rst                              
  ...
  counter_256 switch_ctrl(.in_start(in_start_stage0), .counter_out(counter_out_w), .clk(clk), .rst(rst));

  assign wire_ctrl_stage0[0] = counter_out_w[7]; 
  assign wire_ctrl_stage0[1] = counter_out_w[7]; 
  ...
  assign wire_ctrl_stage1[0] = counter_out_w[6]; 
  assign wire_ctrl_stage1[1] = counter_out_w[6]; 
...
rst                              
  ...
  wire [7:0] counter_out_w;
  counter_256 switch_ctrl(.in_start(in_start_stage1), .counter_out(counter_out_w), .clk(clk), .rst(rst));
...
module  mem_addr_gen_dp4_mem0_per3(
counter_in,                              
clk,                             
rst,                             
addr_out                            
  ...
  assign word_counter_w[0] = word_counter[0];    
      assign addr_out[3:0] = word_counter_w[0][0:3];    
      assign ms_word_high_w = ms_word_high;    
  assign addr_out[7:6] = ms_word_high_w[1:0];    
      assign ms_word_low_w = ms_word_low;    
  assign addr_out[5:4] = ms_word_low_w[0:1];   
  ...
  begin                            
    if(rst) begin                    
      ...
      ms_word_low <= (counter_in[2:0] == {3{1'b1}}) + ms_word_low; 
      ms_word_high <= (counter_in[4:0] == {5{1'b1}}) + ms_word_high; 
      end
  end                              
...
rst                              
  ...
  mem_addr_gen_dp4_mem0_per3 top_mem_addr_gen_inst (.clk(clk),.rst(rst),.addr_out(mem_addr_out_w), .counter_in(counter)); 

  assign mem_addr_out_w_h = mem_addr_out_w[7:6];        
  ...
  assign out_start = (state == 2'b01) && (counter[7:0] == {8{1'b1}}); 
    ...
    if(rst) begin                    
      ...
      case (state)              
        ...
        2'b01: begin              
          ...
          if (!in_start && counter == {8{1'b1}})  begin 
            state <= 2'b11;              
          end
...
rst                              
  ...
  mem_addr_ctrl_dp4_per3 addr_gen_inst(.in_start(in_start), .wen_out(wen_wire), .out_start(out_start_wire), .mem_addr_out_0(addr_wire_0), .mem_addr_out_1(addr_wire_1), .mem_addr_out_2(addr_wire_2), .mem_addr_out_3(addr_wire_3), .clk(clk), .rst(rst));

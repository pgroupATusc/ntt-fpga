...
rst                              
  ...
  wire [8:0] counter_out_w;
  counter_512 switch_ctrl(.in_start(in_start_stage2), .counter_out(counter_out_w), .clk(clk), .rst(rst));
...
module  mem_addr_gen_dp8_mem0_per4(
counter_in,                              
clk,                             
rst,                             
addr_out                            
  ...
  assign word_counter_w[1] = counter_in[4:1];  
  assign ms_word_low_w = counter_in[5:4]; 
  assign ms_word_high_w = counter_in[6:5]; 
    
endmodule                        


module  mem_addr_ctrl_dp8_per4(
in_start,                          
wen_out,                         
out_start,                         
mem_addr_out_0,                         
...
rst                              
  ...
  mem_addr_gen_dp8_mem0_per4 top_mem_addr_gen_inst (.clk(clk),.rst(rst),.addr_out(mem_addr_out_w), .counter_in(counter)); 

  assign mem_addr_out_w_h = mem_addr_out_w[8:6];        

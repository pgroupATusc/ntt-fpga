...
rst                              
  ...
  switches_stage_st0_6_L switch_stage_0(
  ...
  assign wire_ctrl_stage0[0] = counter_w[12]; 
...
rst                              
  ...
  switches_stage_st0_6_R switch_stage_0(
  ...
  always@(posedge clk)             
  begin                            
    wire_ctrl_stage0[0] <= counter_w[12]; 
  end                            
...
module  mem_addr_gen_dp2_mem0_per6(
counter_in,                              
clk,                             
rst,                             
addr_out                            
  ...
  wire [3:0] word_counter_w [2:0];      
  wire [0:0] addr_a0;      
  assign addr_out[3:0] = {word_counter_w[2][0],word_counter_w[2][1],word_counter_w[2][2],word_counter_w[2][3]};    
  assign addr_out[7:4] = {word_counter_w[1][0],word_counter_w[1][1],word_counter_w[1][2],word_counter_w[1][3]};    
  assign addr_out[11:8] = {word_counter_w[0][0],word_counter_w[0][1],word_counter_w[0][2],word_counter_w[0][3]};    
  assign addr_out[12:12] = addr_a0[0:0];   
  
  assign word_counter_w[0] = counter_in[3:0];  
  assign word_counter_w[1] = counter_in[7:4];  
  assign word_counter_w[2] = counter_in[11:8];  
  assign addr_a0 = counter_in[12:12]; 
  
endmodule                        


module  mem_addr_ctrl_dp2_per6(
in_start,                          
counter_in,                         
wen_out,                         
out_start,                         
...
rst                              
  ...
  input in_start, clk, rst;                   
  input [12:0] counter_in; 
  output [12:0] mem_addr_out_0;            
  output [12:0] mem_addr_out_1;            
  output wen_out;
  ...
  wire [12:0] mem_addr_out_tmp_0;   
  wire [12:0] mem_addr_out_tmp_1;   
  wire [12:0] mem_addr_out_w;        
  wire [0:0] mem_addr_out_w_h;        
  wire [11:0] mem_addr_out_w_l;        
  
  mem_addr_gen_dp2_mem0_per6 top_mem_addr_gen_inst (.clk(clk),.rst(rst),.addr_out(mem_addr_out_w), .counter_in(counter_in)); 

  assign mem_addr_out_w_h = mem_addr_out_w[12:12];        
  assign mem_addr_out_w_l = mem_addr_out_w[11:0];   
  ...
  begin                            
    if(rst) begin                    
      ...
      out_start <= (state == 2'b01) && (counter_in[12:0] == {13{1'b1}}); 
end                              
end                              
  ...
  begin                            
    if(rst) begin                    
      ...
      case (state)              
        ...
        2'b01: begin              
          if (!in_start && counter_in == {13{1'b1}})  begin 
            state <= 2'b11;              
          end
          if (counter_in == {13{1'b1}})  begin 
            flag <= !flag;              
          end
        end
        2'b11: begin              
...
rst                              
  ...
  wire [12:0] addr_wire_0;        

  wire [12:0] addr_wire_1;        

  mem_addr_ctrl_dp2_per6 addr_gen_inst(.in_start(in_start), .counter_in(counter_in), .wen_out(wen_wire), .out_start(out_start_wire), .mem_addr_out_0(addr_wire_0), .mem_addr_out_1(addr_wire_1), .clk(clk), .rst(rst));

  block_ram_sp #(.DATA_WIDTH(32), .ADDR_WIDTH(13)) 
...
rst                              
  ...
  wire [12:0] counter_out_w;               
  assign wireIn[0] = inData_0;    
  assign wireIn[1] = inData_1;    
  
  counter_16384 ctrl_unit(.in_start(in_start), .counter_out(counter_out_w), .clk(clk), .rst(rst));

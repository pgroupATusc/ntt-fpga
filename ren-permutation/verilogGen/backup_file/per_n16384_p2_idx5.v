...
rst                              
  ...
  counter_8192 switch_ctrl(.in_start(in_start_stage0), .counter_out(counter_out_w), .clk(clk), .rst(rst));

  assign wire_ctrl_stage0[0] = counter_out_w[12]; 
...
module  mem_addr_gen_dp2_mem0_per5(
counter_in,                              
clk,                             
rst,                             
addr_out                            
  ...
  assign addr_out[11:8] = {word_counter_w[2][0],word_counter_w[2][1],word_counter_w[2][2],word_counter_w[2][3]};    
  assign addr_out[12:11] = ms_word_high_w[1:0];    
  assign addr_out[13:12] = {ms_word_low_w[0],ms_word_low_w[1]};   
  
  assign word_counter_w[0] = counter_in[3:0];  
  assign word_counter_w[1] = counter_in[6:3];  
  assign word_counter_w[2] = counter_in[10:7];  
  assign ms_word_low_w = counter_in[11:10]; 
  assign ms_word_high_w = counter_in[12:11]; 
    
endmodule                        


module  mem_addr_ctrl_dp2_per5(
in_start,                          
wen_out,                         
out_start,                         
mem_addr_out_0,                         
...
rst                              
  ...
  mem_addr_gen_dp2_mem0_per5 top_mem_addr_gen_inst (.clk(clk),.rst(rst),.addr_out(mem_addr_out_w), .counter_in(counter)); 
  ...
  assign out_start = (state == 2'b01) && (counter[12:0] == {13{1'b1}}); 
    ...
    if(rst) begin                    
      ...
      case (state)              
        ...
        2'b01: begin              
          ...
          if (!in_start && counter == {13{1'b1}})  begin 
            state <= 2'b11;              
          end
...
rst                              
  ...
  wire [12:0] addr_w_wire_0;        

  wire [12:0] addr_w_wire_1;        

  wire [12:0] addr_r_wire_0;        

  counter_8192 counter_inst(.in_start(in_start), .counter_out(addr_r_wire_0), .clk(clk), .rst(rst));

  mem_addr_ctrl_dp2_per5 addr_gen_inst(.in_start(in_start), .wen_out(wen_wire), .out_start(out_start_wire), .mem_addr_out_0(addr_wire_0), .mem_addr_out_1(addr_wire_1), .clk(clk), .rst(rst));

  block_ram_dp #(.DATA_WIDTH(32), .ADDR_WIDTH(13)) 
